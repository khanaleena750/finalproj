  ...
  function Survey(survey) {
    ...
    function handleFormSubmit(e) {
      ...
      const answeredQuestions = Object.keys(formData).filter(
        (key) => formData[key].hasOwnProperty('answer')
      );

      if (answeredQuestions.length < Object.keys(formData).length) {
        // If not all questions have been answered, display an error message
        showError(
          currentPanel.querySelector('.error-message'),
          'Please answer all questions before submitting the survey.'
        );
        return;
      }

      // If all questions have been answered, send the survey responses to the server
      fetch('/survey', {
        method: 'POST',
        headers: {
        ...
        .then((data) => {
          if (data.success) {
            // If the survey response was saved successfully, display a success message
            ...
            // If there was an error saving the survey response, display an error message
            showError(
              currentPanel.querySelector('.error-message'),
